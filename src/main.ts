...
type Ticket={id:string;name:string;price:number;items:string[]};
...
let selected=tickets[0],method:'mpesa'|'card'='mpesa',poll=0 as any;
const app=document.querySelector<HTMLDivElement>('#app')!;

const validGmail=(v:string)=>/^[^\s@]+@gmail\.com$/i.test(v.trim());
const validPhone=(v:string)=>/^((0?7\d{8})|(2547\d{8})|(\+2547\d{8}))$/.test(v.replace(/\s/g,''));
const ref=(p:string)=>`${p}-${Date.now()}-${Math.random().toString(36).slice(2,7)}`;
...
function bind(){
 ...
 document.querySelectorAll('[data-close]').forEach(x=>x.addEventListener('click',()=>document.querySelector('#'+(x as HTMLElement).dataset.close!)?.classList.add('hidden')));
 document.querySelectorAll<HTMLButtonElement>('[data-ticket]').forEach(b=>b.addEventListener('click',()=>openTicket(b.dataset.ticket!)));
 document.querySelector('#nameNext')?.addEventListener('click',()=>{const v=(document.querySelector('#name') as HTMLInputElement).value.trim();if(v.length<2)return status('ticketStatus','Enter your full name.','error');step(2)});
 document.querySelector('#emailNext')?.addEventListener('click',()=>{const v=(document.querySelector('#email') as HTMLInputElement).value;if(!validGmail(v))return status('ticketStatus','Enter a valid Gmail address.','error');step(3)});
 document.querySelectorAll<HTMLButtonElement>('[data-method]').forEach(b=>b.addEventListener('click',()=>{method=b.dataset.method as any;document.querySelectorAll('[data-method]').forEach(x=>x.classList.toggle('active',(x as HTMLElement).dataset.method===method));document.querySelector('#phoneBox')?.classList.toggle('hidden',method==='card');(document.querySelector('#pay') as HTMLButtonElement).textContent=method==='card'?'Continue to Card Payment':'Pay with M-PESA'}));
...
function openTicket(id:string){selected=tickets.find(x=>x.id===id)!;method='mpesa';(document.querySelector('#checkout')!).classList.remove('hidden');(document.querySelector('#ticketTitle')!).textContent=`${selected.name} — KSh ${selected.price.toLocaleString()}`;step(1);updateTotal()}
function step(n:number){['s1','s2','s3'].forEach((x,i)=>document.querySelector('#'+x)?.classList.toggle('hidden',i!==n-1));}
function updateTotal(){const q=Math.max(1,Math.min(20,Number((document.querySelector('#qty') as HTMLInputElement)?.value)||1));(document.querySelector('#total')!).textContent=`KSh ${(selected.price*q).toLocaleString()}`}
function status(id:string,msg:string,type:'error'|'success'|'pending'){const e=document.querySelector('#'+id)!;e.textContent=msg;e.className=`status ${type}`;e.classList.remove('hidden')}
async function call(body:any){const r=await fetch(API,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const d=await r.json().catch(()=>({}));if(!r.ok||!d.success)throw new Error(d.error||d.message||'Payment could not be started.');return d}
function wait(refId:string,isDonation=false){
...
async function ticketPay(e:SubmitEvent){e.preventDefault();const name=(document.querySelector('#name') as HTMLInputElement).value.trim(),email=(document.querySelector('#email') as HTMLInputElement).value.trim(),phone=(document.querySelector('#phone') as HTMLInputElement).value.trim(),q=Math.max(1,Math.min(20,Number((document.querySelector('#qty') as HTMLInputElement).value)||1)),amount=selected.price*q;
 ...
 const b=document.querySelector('#pay') as HTMLButtonElement;b.disabled=true;b.textContent='Processing…';
 ...
 if(method==='card'&&d.access_code){const s=document.createElement('script');s.src='https://js.paystack.co/v2/inline.js';s.onload=()=>{const P:any=(window as any).PaystackPop;if(!P)throw new Error('Paystack checkout unavailable');const p=new P();p.resumeTransaction(d.access_code);wait(d.reference||d.data?.reference||'');};document.head.appendChild(s)}
